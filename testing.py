...
def generating_metadata_compleate() -> (str, pd.Timestamp,dict):
    """
...
def test_load_data():
    ...
    GIVEN: a path and a deadline timestamp generated by the function generating_metadata_compleate().
    WHEN: loading data using load_data() function.
    THEN: the function should return a pandas DataFrame.
    """

    path,deadline,dtype=generating_metadata_compleate()
...
def test_load_graph_connected():
    ...
    assert np.array_equal((tail @ head.T).toarray(),adj.toarray())
    assert np.array_equal(users,usermap)
